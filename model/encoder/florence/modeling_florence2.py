...
class ChannelAttention(nn.Module):
    ...
    def __init__(self, dim, groups=8, qkv_bias=True, use_flashattn=False):
        super().__init__()
    ...
    def forward(self, x, size):
        ...
        if self.use_flashattn:
            x = flash_attn_func(q, k, v, 0, softmax_scale=None, causal=False)
        else:
            q = q * (float(N) ** -0.5)
...
class ChannelBlock(nn.Module):
    ...
    def __init__(self, dim, groups, mlp_ratio=4., qkv_bias=True,
                 ...
                 conv_at_attn=True, conv_at_ffn=True, use_flashattn = False):
        super().__init__()
        ...
        self.channel_attn = PreNorm(
            norm_layer(dim),
            ChannelAttention(dim, groups=groups, qkv_bias=qkv_bias, use_flashattn=use_flashattn),
            drop_path
        )
...
class WindowAttention(nn.Module):
    def __init__(self, dim, num_heads, window_size, qkv_bias=True, use_flashattn=False):
...
class SpatialBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size,
                 ...
                 norm_layer=nn.LayerNorm, conv_at_attn=True, conv_at_ffn=True,use_flashattn=False):
        super().__init__()
        ...
        self.window_attn = PreNorm(
            norm_layer(dim),
            WindowAttention(dim, num_heads, window_size, qkv_bias=qkv_bias,use_flashattn=use_flashattn),
            drop_path
        )
        self.conv2 = PreNorm(None, DepthWiseConv2d(
            dim, 3, 1, 1)) if conv_at_ffn else None
...
class Florence2VisionModel(Florence2PreTrainedModel):
    def __init__(self, config: Florence2VisionConfig):
        super().__init__(config)
        assert config.model_type == 'davit', f"only DaViT is supported for now, we you give is {config.model_type}"
...
class Florence2VisionModelWithProjection(Florence2PreTrainedModel):
    ...
    def forward(self, pixel_values):
        ...
        if self.image_pos_embed is not None:
            ...
            #assert h * w == num_tokens, f'only support square feature maps for now, get h={h} w={w} and num_tokens={num_tokens}'
            h, w = self.config.image_embedding_size
        ...
        x = temporal_avg_pool_x - spatial_avg_pool_x 
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    def __init__(self, config: Florence2Config):
        super().__init__(config)
        assert config.vision_config.model_type == 'davit', f"only DaViT is supported for now, we you give is {config.model_type}"
...
AutoModel.register(Florence2VisionConfig, Florence2VisionModelWithProjection)
