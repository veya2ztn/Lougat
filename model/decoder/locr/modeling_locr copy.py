...
class PromptBartForCausalLM(PromptBartPreTrainedModel):   
    ...
    def forward(
        ...
        #print(f"cross_attn_weights.shape={cross_attn_weights.shape}")
        prompt_pred = self.decode_position(cross_attn_weights,attention_mask, full_prompt_in = full_prompt_in, image_tensors = image_tensors if self.config.use_image_bias else None)
