...
class PositionEmbeddingRandom(nn.Module):
    ...
    def _pe_encoding(self, coords: torch.Tensor, omit_ratio=0) -> torch.Tensor:
        ...
        # the locr is not designed for this case, in factor, its input is not correct in strict.
        # assert coords.min() >= 0
    ...
    def forward(self, size: Tuple[int, int]) -> torch.Tensor:
        ...
        h, w = size
        grid = torch.ones((h, w),dtype=torch.float32,device=self.positional_encoding_gaussian_matrix.device)
