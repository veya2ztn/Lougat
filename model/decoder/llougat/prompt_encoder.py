...
class PromptEncoder(nn.Module):
    def __init__(
        ...
        if use_start_pos_embedding:
            self.start_pos_embedding = nn.Parameter(torch.randn(1,embed_dim))
    ...
    def _embed_boxes(self, boxes: torch.Tensor,omit_ratio=0) -> torch.Tensor:
        ...
        #boxes = boxes + 0.5  # Shift to center of pixel  #### < --- lets do not shift it 
        ...
        if self.use_start_pos_embedding:
            is_start_pos_per_batch = (boxes[:,0].view(-1, 4) == 0).all(dim=1)
            corner_embedding[is_start_pos_per_batch,0,:,:] += self.start_pos_embedding ## somehow can only use += for autocast mode.
    ...
    def forward(
        self,
        points: Optional[torch.Tensor],
        boxes: Optional[torch.Tensor], #(bs, -1, 2, 2)
        ...
        if boxes is not None:
            ...
            if add_noise:
                boxes = self.add_random_noise(boxes)    #  [bs,len,2,2] ## notice the noise only work for input, the output is still the correct bbox
            ...
            if sparse_embeddings is None:
                ...
                sparse_embeddings = torch.cat([sparse_embeddings, box_embeddings], dim=-2)  #[bs,len,n+1,d]
        #sparse_embeddings = sparse_embeddings.flatten(-2, -1)  # [bs,len,2,d] -> [bs,len,d]
        sparse_embeddings = sparse_embeddings.sum(-2)  # -> [bs,len,d]
...
class PositionEmbeddingRandom(nn.Module):
    ...
    def _pe_encoding(self, coords: torch.Tensor, omit_ratio=0) -> torch.Tensor:
        ...
        # assert coords.min() >= 0, f"your min coords value up to {coords.min()}"
        # assert coords.max() <= 1, f"your max coords value up to {coords.max()}"
        coords_emb = 2 * coords - 1 # coords: [bs*seq_len,2,2] ## from [0,1] to [-1,1] and [0,0,0,0] will be [-1,-1,-1,-1]
        coords_emb = coords_emb.to(self.positional_encoding_gaussian_matrix)
    ...
    def forward(self, size: Tuple[int, int]) -> torch.Tensor:
        ...
        h, w = size
        grid = torch.ones((h, w)).to(self.positional_encoding_gaussian_matrix)
        y_embed = grid.cumsum(dim=0) - 0.5
        x_embed = grid.cumsum(dim=1) - 0.5
        y_embed = y_embed / h
        x_embed = x_embed / w
        coord   = torch.stack([x_embed, y_embed], dim=-1)
        
        pe = self._pe_encoding(coord)
